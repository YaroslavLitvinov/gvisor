...
func (fd *controlFDLisa) Open(c *lisafs.Connection, comm lisafs.Communicator, flags uint32) (lisafs.FDID, error) {
	flags |= openFlags
	...
	return openFD.ID(), nil
}

// OpenCreate implements lisafs.ControlFDImpl.OpenCreate.
func (fd *controlFDLisa) OpenCreate(c *lisafs.Connection, comm lisafs.Communicator, mode linux.FileMode, uid lisafs.UID, gid lisafs.GID, name string, flags uint32) (lisafs.Inode, lisafs.FDID, error) {
...
func (fd *controlFDLisa) ListXattr(c *lisafs.Connection, size uint64) (uint32, error) {
	return 0, unix.EOPNOTSUPP
}
