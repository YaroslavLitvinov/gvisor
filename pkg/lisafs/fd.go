...
type ControlFDImpl interface {
	...
	Open(c *Connection, comm Communicator, flags uint32) (FDID, error)
	OpenCreate(c *Connection, comm Communicator, mode linux.FileMode, uid UID, gid GID, name string, flags uint32) (Inode, FDID, error)
	...
	ListXattr(c *Connection, size uint64) (uint32, error)
